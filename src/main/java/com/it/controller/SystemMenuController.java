...
public class SystemMenuController {
	...
	@ResponseBody
	public List<Tree> getMenu(Tree tree, HttpServletRequest req) {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if(list.get(i).getPid() == 0) {
				Tree t1 = new Tree();
				t1.setId(list.get(i).getId().toString());
				t1.setText(list.get(i).getName());
//				t1.setState("closed");
				t1.setPermission(list.get(i).getPermission());
				//找出二级菜单
				List<Tree> menuList1 = new ArrayList<Tree>();
				for (int j = 0; j < list.size(); j++) {
					if(list.get(i).getId() == list.get(j).getPid()) {
						Tree t = new Tree();
						t.setId(list.get(j).getId().toString());
						t.setText(list.get(j).getName());
//						t.setState("closed");
						t.setPermission(list.get(j).getPermission());
						//找出三级菜单
						List<Tree> menuList3 = new ArrayList<Tree>();
						for (int k = 0; k < list.size(); k++) {
							if(list.get(j).getId() == list.get(k).getPid()){
								Tree t2 = new Tree();
								t2.setId(list.get(k).getId().toString());
								t2.setText(list.get(k).getName());
//								t2.setState("closed");
								t2.setPermission(list.get(k).getPermission());
								menuList3.add(t2);
							}
						}
						t.setChildren(menuList3);
						menuList1.add(t);
					}
				}
				t1.setChildren(menuList1);
				menuList.add(t1);
			}
		}
		return menuList;	
